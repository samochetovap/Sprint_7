...
public class CourierLoginTest {
    ...
    public void courierRequiredFieldsLoginTest() {
        Courier courierOnlyPassword = new Courier(null, "13213213213", "name");
        Response responseOnlyPassword = given()
                .contentType(ContentType.JSON).and().body(courierOnlyPassword).and()
                .post("/api/v1/courier/login");
        responseOnlyPassword.then().assertThat().statusCode(400).and().body("message", is("Недостаточно данных для входа"));
        Courier courierOnlyLogin = new Courier("13213213213", "", "name");
        Response responseOnlyLogin = given()
                .contentType(ContentType.JSON).and().body(courierOnlyLogin).and()
                .post("/api/v1/courier/login");
        responseOnlyLogin.then().assertThat().statusCode(400).and().body("message", is("Недостаточно данных для входа"));
    }
